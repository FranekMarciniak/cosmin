...
function createScrambler() {
  ...
  const generateCounters = (length) =>
    Array.from(
      { length },
      () => Math.floor(Math.random() * state.maxRandomizationSteps) + 1
    );
  ...
  const scrambleText = (text, callback) => {
    ...
    animate(() => {
      if (state.scrambledText.length !== text.length) {
        updateText(
          generateRandom(
            state.scrambledText.length < text.length
              ? state.scrambledText.length + 1
              : text.length
          )
        );
  ...
  return {
    updateMaxRandomizationSteps: (steps) =>
      (state.maxRandomizationSteps = steps),
    resetScrambler: () =>
      Object.assign(state, {
        originalText: "",
        scrambledText: "",
...
function scrambleElements(className) {
  ...
  elements.forEach((element, index) => {
    ...
    scrambler.scrambleText(initialText, (updatedText) => {
...
class Carousel {
  ...
  createSlidingStructure() {
    ...
    this.slides.forEach((slide, index) => {
      ...
      if (slide.image) {
        if (slide.image.path) {
          img.src = slide.image.path; // explicit path
        } else {
          const card = slide.image.card || "";
          const name = slide.image.name || "";
          const widths = slide.image.widths || [
  ...
  touchMove(event) {
    ...
    if (
      ...
      (this.totalMovement > 10 || this.totalMovementY > 10)
    ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (document.getElementById("retail-carousel") && recentWorkSlides.length) {
    new Carousel("retail-carousel", recentWorkSlides, [
      "bg-red",
      "bg-blue",
      "bg-emerald",
      "bg-yellow",
